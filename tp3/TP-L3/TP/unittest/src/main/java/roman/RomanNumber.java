...
public final class RomanNumber extends Number {
  ...
  private static int fromRoman(String romanValue) {
    int resultat = 0;
		int index = 0;
		String sousChaine;
		for (Map.Entry mapentry : SYMBOLS.entrySet()) {
			sousChaine=romanValue.substring(index,index+mapentry.getKey().length());
			while(sousChaine.equals(mapentry.getKey())){
				resultat+=mapentry.getValue();
				index+=mapentry.getKey().length();
				sousChaine="";
			}
		}
    return resultat;
  ...
  private static String toRoman(int value) {
    ...
    for (Map.Entry mapentry : SYMBOLS.entrySet()) {
			//Tant que la value est >= au nombre 
